...
describe('CarouselFadeExample', () => {
    it('renders the carousel and checks for images and captions', () => {
        ...
        expect(images[0]).toHaveAttribute('src', expect.stringContaining('property.jpeg'));
        expect(images[1]).toHaveAttribute('src', expect.stringContaining('property1.png'));
        expect(images[2]).toHaveAttribute('src', expect.stringContaining('property2.jpeg'));
        ...
        expect(screen.getByText('Nulla vitae elit libero, a pharetra augue mollis interdum.')).toBeInTheDocument();
        expect(screen.getByText('Lorem ipsum dolor sit amet, consectetur adipiscing elit.')).toBeInTheDocument();
        expect(screen.getByText('Praesent commodo cursus magna, vel scelerisque nisl consectetur.')).toBeInTheDocument();
    });
