  ...
  const getUnitsFromProperties = (properties, companyId) => {
    ...
    Object.values(properties).forEach(property => {
      ...
      ;[condo_units, parking_units, storage_units].forEach(unitsArray => {
        if (unitsArray && Array.isArray(unitsArray)) {
          // Ensure unitsArray exists and is an array
          allUnits = [...allUnits, ...unitsArray]
        }
      })
      // }
    })
    const unitsWithNullUser = allUnits.filter(
      unit => unit.public_profile === null,
    )
  ...
  const SendRegistrationkeyRequest = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post(
        '/registration-keys/condo-registration-key/',
        {
      ...
      <Modal show={showModal} onHide={handleClose} centered>
        <Modal.Header closeButton>
          ...
          <Modal.Title className='w-100 text-center'>
            <h2>Registration Key Details</h2>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          ...
          <Dropdown className='mb-4 w-75 text-center mx-auto'>
            <Dropdown.Toggle
              ...
              className='w-100'
            >
              {selectedUnit === 'Your Unit'
                ? 'Your Unit'
                : `Unit ${selectedUnit.location}`}
            </Dropdown.Toggle>
